...
void ofPgBaseProject::setup(string _target){
    target = _target;
    templatePath = ofToDataPath("templates/" + target + "/", true);
	projectTypeName = "undefined";
    setup(); // call the inherited class setup(), now that target is set.
}

bool ofPgBaseProject::create(string path){
...
bool ofPgBaseProject::save(bool createMakeFile){
...
void ofPgBaseProject::addAddon(ofAddon & addon){
...
void ofPgBaseProject::parseAddons(){
